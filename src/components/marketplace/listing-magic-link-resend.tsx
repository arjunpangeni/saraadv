...
export function ListingMagicLinkResend({
  email,
  listingId,
}: {
  email?: string;
  ...
  async function send(address: string) {
    if (!listingId) {
      toast.error("Missing listing. Open the request from the listing page.");
      return;
    }
    if (wait > 0) return;
    setLoading(true);
    const res = await fetch("/api/listing-leads/resend-link", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ email: address, listingId }),
    });
  ...
  const blocked = loading || !listingId || wait > 0;
  const label = loading ? (
    <>
  ...
  return (
    <form onSubmit={onSubmit} className="mt-4 space-y-2 text-left">
      <div className="space-y-1.5">
        <Label htmlFor="listing-resend-email">Work email</Label>
        <Input
          id="listing-resend-email"
          type="email"
          required
          autoComplete="email"
          value={value}
